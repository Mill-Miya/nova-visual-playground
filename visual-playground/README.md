...
白〜青白の不透明核、暗い内部、明るい薄膜を重ねたシアンの球体に、中心を揃えた同心円リングと主アンバー点1個を組み合わせています。傾いた楕円軌道は使わず、円形リングの切れ目・輝度・弧の長さに差を付け、左右対称すぎる印象を和らげています。内部には少数の白金色・淡いアンバーの粒子と暖色の反射を加えています。Idleの呼吸は6秒周期・±2%、外周の姿勢変化はほぼ静止、粒子は極低速です。

状態が変わると内部粒子と外周点が約0.85秒で一周し、その後ゆっくりした周回へ戻ります。連続切替でも現在の位置から開始し、reduced-motion時は一周演出を省略します。内部粒子は小さく淡くし、球面と最外周リングも細く薄くしています。奥側では小さく暗く、手前では大きく明るくなる3D投影を使い、球体内部の深度を示します。Thinking中は回転を静めて中心へ収束し、reduced-motion時は静止します。

Coreから300px以内のpointermoveに対し中心核だけ最大4px移動します。本体の位置と外周アンバー点は追従しません。reduced-motion時は追従しません。

## 状態ごとの視覚とHUD

| 状態 | 演出 | HUD |
| --- | --- | --- |
| Idle | 静かな呼吸、微粒子、濃淡を付けた円形リングと1点 | なし |
